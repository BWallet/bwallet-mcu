...
static void send_msg_features(usbd_device *dev)
{
	...
	while ( usbd_ep_write_packet(dev, 0x81,
		...
		"\x0a\x11" "mybwallet.com\x10" VERSION_MAJOR_CHAR "\x18" VERSION_MINOR_CHAR " " VERSION_PATCH_CHAR "(\x01"		// data
			...
			flash_state = STATE_END;
				switch (language) {
					case CHINESE:
						...
						break;
					default :
						layoutDialog(DIALOG_ICON_WARNING, NULL, NULL, NULL, "Firmware installation", "aborted.", NULL, "You may now", "unplug your BWALLET.", NULL);
						break;
				}
			return;
		}
	...
	if (flash_state == STATE_FLASHSTART) {
		if (msg_id == 0x0007) {		// FirmwareUpload message (id 7)
			if (buf[9] != 0x0a) { // invalid contents
				...
				switch (language) {
					case CHINESE:
						...
						break;
					default :
						layoutDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "Error installing ", "firmware.", NULL, "Unplug your BWALLET", "and try again.", NULL);
						break;
				}
				return;
			}
			...
			if (flash_len > FLASH_TOTAL_SIZE + FLASH_META_DESC_LEN - (FLASH_APP_START - FLASH_ORIGIN)) { // firmware is too big
				...
				switch (language) {
					case CHINESE:
						layoutZhDialog(DIALOG_ICON_WARNING, NULL, NULL, NULL, "固件文件太大#,#请到", NULL, "mybwallet.com", "获取安装文件#.#");
						break;
					default :
						layoutDialog(DIALOG_ICON_ERROR, NULL, NULL, NULL, "Firmware is too big.", NULL, "Get official firmware", "from mybwallet.com", NULL, NULL);
						break;
				}
				return;
			}
		...
		if (hash_check_ok) {
			switch (language) {
				case CHINESE :
					...
					break;
				default :
					layoutDialog(DIALOG_ICON_OK, NULL, NULL, NULL, "New firmware", "successfully installed.", NULL, "You may now", "unplug your BWALLET.", NULL);
					break;
			}
			send_msg_success(dev);
		} else {
